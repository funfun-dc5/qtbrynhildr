...
* [新規機能]
 ...
 2. ステータスバーへのフレームレート(FPS)表示機能を追加

   メニュー項目を追加

 3. ソフトウェア・キーボード/ボタンのテスト実装のマージ

* [その他変更]

 1. ログファイルの作成先をテンポラリディレクトリ下に変更

    UNIX系環境では環境変数TMPDIR下 or /tmp下, Windows環境では%TEMP% or %TMP%下

 2. インストーラパッケージ作成ツールをバージョンアップ(InnoSetup 5.5.6) (Windows版)

 3. Windows 10 (x64/x86)をサポートOSに追加
...
* [不具合修正]
 ...
 2. 変数の未初期化により起動時に内部エラーとなる場合がある不具合修正 (Ver.1.2.0 RC2/3のみ)
