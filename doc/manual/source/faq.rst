...
3. Mac版で内蔵スピーカを利用した時にサウンドにプチノイズが入るのですが?
    ...
    * Launchpad から「その他」を開いて「Audio MIDI設定」を開いて出力のフォーマットを確認してください。

      サーバのサンプリング周波数と出力フォーマットのサンプリング周波数が異なっていませんか?
      異なっている場合再生時に出力フォーマットのサンプリング周波数にリサンプリングされるため
      データが間に合わず無音状態が発生します、このためプチノイズとして聴こえます。
